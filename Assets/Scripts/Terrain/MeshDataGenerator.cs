...
//        }
    ...
    {
		Debug.Log("Starting Mesh Data generation...");
    ...
    private int CreateDataMeshForBlock(int blockX, int blockY, int blockZ, Chunk chunk, int x, int y, int z, int index)
    {
		...
		//string generatedSides = "Block Mesh Sides: ";
		
		if(m_WorldData.DoesBlockExist(blockX, blockY - 1, blockZ))
		{
	        // "South" side
...
//		max_blockX = Math.Max(blockX, max_blockX);
		...
		if(m_WorldData.DoesBlockExist(blockX, blockY, blockZ + 1))
		{
	        // Block above
                              ...
                              byte blockLight)
    {
        ...
        Color item = new Color(actualColor, actualColor, actualColor);
