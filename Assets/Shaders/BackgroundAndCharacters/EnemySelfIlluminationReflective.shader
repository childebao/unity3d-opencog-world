...
Shader "Fallaçade/Character/EnemySelfIlluminationReflective" {
	...
	SubShader {
		...
		Pass {
			...
			{
				v2f o;			
				o.color = (unity_LightColor[0] + unity_LightColor[1]) * 0.2; // heuristic for the lighting, works 'ok' in Fallaçade, but is not a general solution
				o.pos = mul(UNITY_MATRIX_MVP, v.vertex);
				o.uv.xy = TRANSFORM_TEX(v.texcoord, _MainTex);
